...
class UtilLogging:
    ...
    def __init__(self, args, log_name=LOG_NAME, is_file=True, is_stream=True, params=None):
        """
        ...
        :param params: 完整参数数据，不为None则将其输出到参数文件
        """

        app_name = args["app_name"]
        app_time = args["app_time"]
        if log_name not in UtilLogging.name_number.keys():
            UtilLogging.name_number[log_name] = 0
        else:
            UtilLogging.name_number[log_name] += 1
        self.log_name = log_name + "_" + str(UtilLogging.name_number[log_name])
        self.rec_name = app_name + "_" + app_time
        # 用当前时间为本次日志命名
