...
def get_fixed_learning_rate(args):
    ...
    直接返回默认的学习率
    """
    lr = fluid.layers.fill_constant(shape=[1], value=args["base_learning_rate"], dtype='float32')
    return lr
    ...
    with fluid.default_main_program()._lr_schedule_guard():
        ...
        with fluid.layers.control_flow.Switch() as switch:
            with switch.case(global_step < WARM_UP_STEP):
                warmup_lr = START_LR + (LR - START_LR) * (global_step / WARM_UP_STEP)
                fluid.layers.tensor.assign(warmup_lr, lr)
            with switch.case(global_step < WARM_UP_STEP + DECAY_STEP):
                decayed_lr = fluid.layers.learning_rate_scheduler.polynomial_decay(
                    learning_rate=WARM_UP_STEP * (LR - END_LR) / DECAY_STEP + LR,
